...
ReadFromZip <- function (cbook, local_path, subset = NA) {
  ...
  # index of files to read, possibly subset
  list_names = names(GetFileName(cbook))
  subset_idx = 1:length(list_names)
  if (!isTRUE(is.na(subset))) {
    subset_idx = which(list_names%in%subset)
  }
  
  # get file names from codebook
  file_names = GetFileName(cbook)[subset_idx]
  archive_names = sapply(strsplit(GetURL(cbook)[subset_idx], '/'),
                         function(x) rev(x)[1])
  cbook_file_subset <- cbook$files[subset_idx]
  ...
  for (i in seq_along(file_names)) {
    tryCatch({
      path_to_zip = paste0(local_path, archive_names[i])
      # unzip file and retrieve path
      temppath <- readr::datasource(unz(path_to_zip, file_names[i]))[[1]]
      temp <- CBookrFWF(temppath, cbook_file = cbook_file_subset[[i]])
      dat[[i]] <- ApplyVarSpec(temp, cbook_file_subset[[i]]); rm(temp)
    }, error = function (e) dat[[i]] <- NA)
  }
  return(dat)
}
...
ApplyVarSpec <- function (x, cbook_file) {
  ...
  for (i in 1:ncol(x)) { # for all the variables in the data
    ...
    # if variable type is character, convert column to character
    if (vars[[i]]$type == 'character') {
      cat(paste('  Convert to character', '\n'))
